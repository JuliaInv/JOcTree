...
if !((length(nf) == 6) && all(nf .> 0))
  error("nf must be an all positive vector of length 6")
end
...
nbsz = minimum(round(Int64,log2([nx, ny, nz])))
...
# start with fine cells around box
i1 = floor(Integer,xa) 
i2 = floor(Integer,xb)
j1 = floor(Integer,ya)
j2 = floor(Integer,yb)
...
for m = 1:nbsz
  ...
  if m > 1
    ...
    q = trues(i4 - i3 + 1, j4 - j3 + 1, k4 - k3 + 1)
